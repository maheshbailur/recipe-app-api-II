...
class ImageUploadTests(TestCase):
    ...
    """teardown (setup() runs at the beggining) runs at the end of the testcase"""
    """This deletes the uploaded image from the system at the end of testcase"""
    def tearDown(self):
    ...
    def test_upload_image(self):
        ...
        """tempfile provides memory for working on image files here"""
        with tempfile.NamedTemporaryFile(suffix='.jpg') as image_file:
            """Image creates square box image of 10X10 pixel in temp memory"""
            img = Image.new('RGB', (10, 10))
            """saving image to the file"""
            img.save(image_file, format='JPEG')
            """by saving, file pointer moves end of the file
            so brng the pointer back to the start of the file"""
            image_file.seek(0)
